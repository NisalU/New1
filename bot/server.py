...
# ── AI pause gate ─────────────────────────────────────────────────────────────
# Set to True when the user switches symbol so the AI loop stops until they
# click "Start AI Analysis". The countdown timer still runs during this state.
_ai_paused: bool = False
...
async def api_set_symbol(request: web.Request) -> web.Response:
    ...
    _active_symbol = sym
    if old != sym:
        # Pause AI loop — user must click "Start AI Analysis" to resume
        _ai_paused = True
        # Clear stale cache so when the user starts, a fresh call fires
        with ai_analyst._lock:
            ai_analyst._cache.pop(sym, None)
        # Tell all connected clients the active symbol changed and AI is paused
        for c in manager.clients:
            c.send({"type": "active_symbol", "symbol": sym})
            c.send({"type": "ai_paused", "symbol": sym})
    return web.json_response({"symbol": sym, "ok": True, "ai_paused": _ai_paused})
...
async def api_ai_start(request: web.Request) -> web.Response:
    """Start AI analysis for the current symbol.

    Called when the user clicks "Start AI Analysis" after switching symbol.
    Clears the pause flag and wakes the AI loop immediately.
    """
    global _ai_paused
    sym = _active_symbol or config.DEFAULT_SYMBOL
    _ai_paused = False
    # Clear cache so the loop runs a fresh call right away
    ...
    if _priority_event:
        _priority_event.set()
    # Notify all clients AI is now active for this symbol
    for c in manager.clients:
        c.send({"type": "ai_started", "symbol": sym})
    return web.json_response({"symbol": sym, "ok": True, "ai_paused": False})
...
async def ws_endpoint(request: web.Request) -> web.WebSocketResponse:
    ...
    try:
        # Declare globals used in this handler at function scope (not nested)
        global _active_symbol, _ai_paused
        ...
        async for frame in ws:
            ...
            if kind == "subscribe":
                ...
                if _valid_symbol(sym) and ivl in config.INTERVALS:
                    ...
                    if old_sym != sym:
                        # Pause AI loop for the new symbol — user clicks Start
                        _ai_paused = True
                        # Clear stale cache so fresh call fires when user starts
                        with ai_analyst._lock:
                            ai_analyst._cache.pop(sym, None)
                        # Tell all clients the active coin changed and AI is paused
                        for c in manager.clients:
                            c.send({"type": "active_symbol", "symbol": sym})
                            c.send({"type": "ai_paused", "symbol": sym})
                    if ai_analyst.enabled:
                        ...
                        if cached_ai:
                            client.send({"type": "ai", "data": cached_ai})
                        # Send current pause state to this client
                        client.send({"type": "ai_paused" if _ai_paused else "ai_started", "symbol": sym})
...
async def _ai_loop() -> None:
    ...
    or the WS "subscribe" message.

    When _ai_paused is True the loop keeps the countdown timer running but
    skips the actual AI call — the user must click "Start AI Analysis" to
    resume.  The timer continues so the user has a visual heartbeat even
    while paused.
    """
    ...
    while True:
        try:
            ...
            # Always update the countdown so the timer keeps ticking
            ...
            # If paused, skip the AI call but keep the loop alive
            if _ai_paused:
                if _priority_event:
...
if __name__ == "__main__":
    # In non-TTY environments (e.g. Replit), read keys from env vars directly.
    # Interactive prompting is skipped; set GROQ_API_KEY via Replit Secrets.
    import sys as _sys
    if _sys.stdin.isatty():
        ...
        if groq_key:
            print("[ai] GROQ_API_KEY loaded from environment.")
        else:
            print("[ai] No GROQ_API_KEY — AI analyst disabled.")
    _load_app_modules()
