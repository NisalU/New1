...
(function () {
  ...
  var _aiPaused       = false;    // true when user changed symbol; shows Start button
  ...
  var aiStartWrapEl = document.getElementById("ai-start-wrap");
  var aiStartBtnEl  = document.getElementById("ai-start-btn");
  var aiStartHintEl = document.getElementById("ai-start-hint");

  function showAIStartButton(hint) {
    if (aiStartWrapEl) aiStartWrapEl.classList.remove("hidden");
    if (aiStartHintEl && hint) aiStartHintEl.textContent = hint;
    if (aiStartBtnEl)  { aiStartBtnEl.disabled = false; aiStartBtnEl.textContent = ""; }
    // Re-add icon + text (textContent wipes children — use innerHTML)
    if (aiStartBtnEl) aiStartBtnEl.innerHTML = '<span class="ai-start-icon">▶</span><span>Start AI Analysis</span>';
  }

  function hideAIStartButton() {
    if (aiStartWrapEl) aiStartWrapEl.classList.add("hidden");
  }

  if (aiStartBtnEl) {
    aiStartBtnEl.addEventListener("click", function () {
      aiStartBtnEl.disabled = true;
      aiStartBtnEl.innerHTML = '<span class="ai-start-icon ai-start-spin">↻</span><span>Starting…</span>';
      fetch("/api/ai/start", { method: "POST" })
        .then(function (r) { return r.json(); })
        .then(function () {
          // Server will broadcast ai_started — button hides when that arrives
          showAIAnalyzing(symbolEl ? symbolEl.value : "");
        })
        .catch(function () {
          aiStartBtnEl.disabled = false;
          aiStartBtnEl.innerHTML = '<span class="ai-start-icon">▶</span><span>Start AI Analysis</span>';
        });
    });
  }
  ...
  function updateAICountdown() {
    ...
    if (_aiPaused) {
      // Counter still ticks but shows paused state visually
      nextAnalysisEl.textContent = "⏸ " + label;
      nextAnalysisEl.className = "countdown-pill paused";
    } else {
    ...
    // Ring keeps animating regardless of pause state
  ...
  function onAICountdown(msg) {
    ...
    // Server may carry pause state in countdown push (e.g. after symbol switch)
    if (msg.ai_paused !== undefined) _aiPaused = msg.ai_paused;
    updateAICountdown();
  }
    ...
    ws.onmessage = function (ev) {
      ...
      switch (m.type) {
        case "config":
          ...
          // Restore pause state from server (e.g. on page reload after symbol switch)
          if (m.ai_paused) {
            _aiPaused = true;
            showAIStartButton("Click to begin AI analysis for " + (symbolEl ? symbolEl.value : ""));
          } else {
            _aiPaused = false;
            hideAIStartButton();
          }
        ...
        case "ai_analyzing":
          ...
          break;
        case "ai_paused":
          // User switched symbol — AI loop stopped; show Start button
          if (!m.symbol || m.symbol === symbolEl.value) {
            _aiPaused = true;
            showAIStartButton("Symbol changed — click to begin analysis for " + symbolEl.value);
          }
          break;
        case "ai_started":
          // User clicked Start — AI loop is running again; hide Start button
          if (!m.symbol || m.symbol === symbolEl.value) {
            _aiPaused = false;
            hideAIStartButton();
            if (m.symbol === symbolEl.value) showAIAnalyzing(m.symbol);
  ...
  function reset() {
    ...
    // On symbol/interval change: pause AI and show Start button
    _aiPaused = true;
    showAIStartButton("Symbol changed — click to begin analysis for " + sym);
  ...
  symbolEl.addEventListener("change",  function () { localStorage.setItem(LS_SYM, symbolEl.value);   reset(); syncScannerActiveRow(); });
  ...
  function renderScanner(data) {
    ...
    var html = "";
    var activeSym = symbolEl ? symbolEl.value : "";
  ...
  // Highlight whichever scanner row matches the currently selected symbol
  function syncScannerActiveRow() {
    var tbody = document.getElementById("scanner-tbody");
    if (!tbody) return;
    var activeSym = symbolEl ? symbolEl.value : "";
  ...
  // Switch dashboard to a coin from the scanner
  function switchToScannerSymbol(sym) {
    if (!sym || !symbolEl) return;
    if (sym === symbolEl.value) return;
    // Add option dynamically if the symbol isn't already in the dropdown
    var found = false;
    for (var i = 0; i < symbolEl.options.length; i++) {
      if (symbolEl.options[i].value === sym) { found = true; break; }
    }
    if (!found) {
