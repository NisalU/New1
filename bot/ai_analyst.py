...
STEP 6b — TURTLE SOUP  [turtle_soup field]
Turtle Soup = false breakout of the 20-period high/low; trapped breakout traders fuel the reversal.
If turtle_soup is present and signal matches your 4H bias:
• Use turtle_soup.entry as your LIMIT entry (at the swept N-period level).
• Use turtle_soup.stop as your stop loss (beyond the sweep wick).
• This is highest-conviction entry — ranks above OB/FVG when 4H aligns.
• set setup_type = "turtle_soup_long" or "turtle_soup_short".
• Confidence boost: +10 if 4H + 1H structure aligned, +5 if prime session.
If turtle_soup signal OPPOSES your 4H bias → ignore it.
...
def _turtle_soup_signal(candles, n=20):
    """Detect a live Turtle Soup (false breakout / false breakdown) entry.

    Original technique: Linda Raschke & Larry Connors.
    When price sweeps the N-period high or low and then closes back inside the
    range within 1-3 bars, the breakout is fake — trapped traders provide
    fuel for the reversal.

    Checks the last 1–3 candles for a qualifying sweep so both just-printed
    and slightly-aged setups are caught.

    Returns a compact dict the AI uses to set the precise entry and stop,
    or None when no setup is present.

    Token cost in the prompt payload: ~25 tokens (compact JSON).
    """
    if len(candles) < n + 4:
        return None
    ...
    for bars_back in range(1, 4):       # check breakout bar 1, 2, or 3 candles ago
        idx_break = -(bars_back + 1)
        idx_win_end = idx_break
        idx_win_start = idx_win_end - n
        if abs(idx_win_start) > len(candles):
            break

        bk     = candles[idx_break]     # the candle that printed the sweep
        window = candles[idx_win_start:idx_win_end]
        if len(window) < n:
            continue
        ...
        # ── Bearish Turtle Soup ────────────────────────────────────────────
        # N-period high swept, price has since closed back below it.
        # Buy-side liquidity cleared → short the trapped longs.
        if bk["high"] > period_high and cur["close"] < period_high:
            entry = _fnum(period_high * 0.9998)   # limit just below the swept level
            stop  = _fnum(bk["high"] + a * 0.20)  # above the sweep wick
            return {
                "signal":           "SHORT",
                "entry":            entry,
                "stop":             stop,
                "period_level":     _fnum(period_high),
                "sweep_extreme":    _fnum(bk["high"]),
                "bars_since_break": bars_back,
                "lookback":         n,
            }

        # ── Bullish Turtle Soup ────────────────────────────────────────────
        # N-period low swept, price has since closed back above it.
        # Sell-side liquidity cleared → long off the trapped shorts.
        if bk["low"] < period_low and cur["close"] > period_low:
            entry = _fnum(period_low * 1.0002)    # limit just above the swept level
            stop  = _fnum(bk["low"] - a * 0.20)   # below the sweep wick
            return {
                "signal":           "LONG",
                "entry":            entry,
                "stop":             stop,
                "period_level":     _fnum(period_low),
                "sweep_extreme":    _fnum(bk["low"]),
                "bars_since_break": bars_back,
                "lookback":         n,
            }
...
def _compact_market(analysis, symbol, regime, structural_quality, memory_rows):
    ...
    return {
        ...
        "turtle_soup":           _turtle_soup_signal(analysis["candles"]),
